/* TODO(cv-tilt): Replace your existing cover image element with:
   <CoverTilt src={imageSrc} alt={alt} />
*/
...
export default function Home(){
  ...
  // injected by script: current volume image + alt
  const imageSrc = (current?.imageBase64 ?? current?.imageUrl ?? current?.image ?? "");
  const alt = `WILD Volume ${current?.volumeNumber ?? current?.volume ?? ""}`;
